...
RUN apt-get update && apt-get install -y vsftpd curl unzip vim liblapack3 && rm -rf /var/lib/apt/lists/*
RUN curl -LO 'http://www.cog-genomics.org/static/bin/plink180528/plink_linux_x86_64.zip' && unzip plink_linux_x86_64.zip && rm plink_linux_x86_64.zip && mv plink_linux_x86_64 /usr/local/bin/
